...
export default function EnterpriseRiskDashboard() {
  ...
  const filteredCategories =
    filterRegulation && REGULATION_RISKS_MAP[filterRegulation]
      ? riskCategories.filter((c) =>
          REGULATION_RISKS_MAP[filterRegulation].includes(c.label),
        )
      : riskCategories;
  ...
  const overallRiskScore =
    filteredCategories.length > 0
      ? Math.round(
          filteredCategories.reduce((acc, curr) => acc + curr.percentage, 0) /
      ...
      <Grid container spacing={3}>
              ...
              >
                ...
                >
                  <Typography
                    ...
                    sx={{
                      fontWeight: 700,
                      color: `${overallRiskLevel.toLowerCase() === "low" ? "success" : overallRiskLevel.toLowerCase() === "high" ? "error" : "warning"}.main`,
                    }}
                  >
                    {overallRiskScore}%
                  </Typography>
          ...
          >
            ...
            >
              <Typography variant="h6" sx={{ fontWeight: 700 }}>
                ...
                {filterRegulation && `(${filterRegulation})`}
              </Typography>
              {filterRegulation && (
                <Chip
                  label="Clear"
                  size="small"
                  onDelete={() =>
                    navigate("/risks", { replace: true, state: {} })
                  }
                  color="primary"
              ...
              {[
                ...
                <Card
                  ...
                  sx={{
                    ...
                    background: theme.palette.background.paper,
                    color: "#E0E0E0",
                    border: `1px solid ${theme.palette[item.color].main}22`,
                  }}
                >
                  <Typography
                    variant="h6"
        ...
        <Grid item xs={12}>
          ...
          >
            ...
            >
              {[65, 72, 68, 55, 48, 45, 42, 44, overallRiskScore].map(
                (val, i) => (
                  <Box
                    key={i}
                    sx={{
